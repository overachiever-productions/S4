...
/*
	...
	- Must be upgraded from previous version
	- or... if nothing is deployed... just run this script. 
		it's idempotent for ... new deployuments
			but... there can be issues if you have, say, version 3.0 and upgrad directly to this without upgrading previous versions
				(that said... i could probably figure out some additional commands to execute based on current version of tables vs desired/target version (i.e., this release))
					that's a v 3.6 feature though.
...
IF OBJECT_ID('dbo.dba_DatabaseBackups_Log','U') IS NOT NULL BEGIN
	...
	ELSE BEGIN;
		...
		-- update the property and do anything else needed: 
		IF @version != '3.5.0.16602' BEGIN;
...
-- RestoreLog:
-----------------------------------------------------------------------------------------------------------
IF OBJECT_ID('dbo.dba_DatabaseRestore_Log','U') IS NOT NULL BEGIN

	DECLARE @version sysname; 
	SELECT @version = CAST([value] AS sysname) FROM sys.extended_properties WHERE major_id = OBJECT_ID('dbo.dba_DatabaseRestore_Log') AND [name] = 'Version';
	...
	IF @version IS NULL BEGIN;
		...
		EXEC sys.sp_addextendedproperty
			...
			@level0name = 'dbo',
			@level1type = 'Table',
			@level1name = 'dba_DatabaseRestore_Log';
...
ELSE BEGIN;
	...
	PRINT 'TABLE dbo.dba_DatabaseRestore_Log Created';
