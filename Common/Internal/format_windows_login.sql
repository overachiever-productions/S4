/*
    ...
    SAMPLES / TESTS 
        ...
        Expected Error: 
                SELECT dbo.format_windows_login(1, N'NONE', N'', '0x0456598165fe', 'Billing', DEFAULT);

        CREATE only - with a (fake) SID and default DB of [Billing] specified
                SELECT dbo.format_windows_login(1, DEFAULT, N'Bilbo', '0x0456598165fe', 'Billing', NULL);

        CREATE, but login defaults to NOT active... so it's disabled: 
                SELECT dbo.format_windows_login(NULL, DEFAULT, N'Bilbo', '0x0456598165fe', 'Billing', NULL);

        CREATE or ALTER (if it exists - but SID can't be changed by the ALTER):
                SELECT dbo.format_windows_login(1, N'ALTER', N'Bilbo', '0x0456598165fe', 'Billing', NULL);

        CREATE or DROP + CREATE 
                SELECT dbo.format_windows_login(1, N'DROP_AND_CREATE', N'Bilbo', '0x0456598165fe', 'Billing', NULL);

        As above, but defaults to SAFE config (i.e., @Enabled is NOT specified):
                SELECT dbo.format_windows_login(NULL, N'DROP_AND_CREATE', N'Bilbo', '0x0456598165fe', 'Billing', NULL);
